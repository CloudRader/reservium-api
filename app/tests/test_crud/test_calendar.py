...
def test_get_created_calendar(calendar_crud, calendar_create):
    ...
    """
    db_calendar = calendar_crud.get(calendar_create.id)
...
def test_delete_calendar(calendar_crud, calendar_create):
    ...
    """
    removed_calendar = calendar_crud.remove(calendar_create.id)
    assert removed_calendar is not None
    assert removed_calendar.id == calendar_create.id
    db_calendar = calendar_crud.get(removed_calendar.id)
...
def test_remove_nonexistent_calendar(calendar_crud, calendar_create):
    ...
    """
    calendar = calendar_crud.create(obj_in=calendar_create)
    calendar_removed = calendar_crud.remove(calendar.id)
    assert calendar_removed is not None
    calendar_removed = calendar_crud.remove(calendar_removed.id)
...
def test_update_calendar(calendar_crud, calendar_create,
    ...
    """
    calendar = calendar_crud.create(obj_in=calendar_create)
    calendar_to_update = calendar_crud.get_by_calendar_id(calendar.id)
