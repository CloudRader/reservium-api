...
class AbstractStudyRoomService(ABC):
    """
...
class StudyRoomService(AbstractStudyRoomService):
    ...
    def post_event(self, event_input: EventInput, user: User, room: Room, creds, services) -> Any:
        ...
        message = self.control_conditions_and_permissions_study_desk(services, event_input, service, calendar)
    ...
    def control_conditions_and_permissions_study_desk(self, services, event_input, service, calendar):
        ...
        if not self.__control_res_in_advance(start_datetime, dt.timedelta(minutes=30)):
            return {"message": "You have to make reservations 30 minutes in advance!"}
    ...
    def control_conditions_and_permissions(self, services, event_input, service, calendar):
        ...
        # Reservation in advance
        if not self.__control_res_in_advance(start_datetime, dt.timedelta(hours=24)):
    ...
    @staticmethod
    def __control_res_in_advance(start_time, time_in_advance):
        current_time = dt.datetime.now()

        time_difference = abs(start_time - current_time)

        if time_difference < time_in_advance:
            return False
        return True
